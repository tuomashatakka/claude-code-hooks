...
export function detectContentLanguage (content: string): SupportedLanguage | null {
  ...
  const checks: Array<[SupportedLanguage, boolean]> = [
...
function detectStructuredLanguage (content: string): SupportedLanguage | null {
...
function highlightJS (code: string): string {
  ...
  return code.replace(tokens, (token, ...args: unknown[]) => {
...
function highlightBash (code: string): string {
  ...
  return code.split('\n').map(line => {
    ...
    return line.replace(tokens, (token, ...args: unknown[]) => {
      const groups = args.at(-1) as Record<string, string | undefined>
      if (groups.string)
        return chalk.green(token)
...
const META_KEY   = chalk.cyan
const META_STR   = chalk.green
