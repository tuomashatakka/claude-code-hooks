...
import { imageToAscii, glyphTable, resolveGlyphMode, type GlyphMode } from '@tuomashatakka/image-to-ascii';
...
for (let i = 0; i < args.length; i++) {
  const arg = args[i]!;
  if (arg === '--width' || arg === '-w') width = Number(args[++i]);
  else if (arg === '--mode' || arg === '-m') mode = args[++i] as GlyphMode;
  else if (arg === '--palette') palette = true;
  else files.push(arg);
}
...
if (mode) process.env.CLAUDE_HOOKS_IMAGE_MODE = mode;
...
for (const glyph of glyphTable(active === 'half' ? 'sextant' : active, palette).glyphs) {
  if (!family.has(glyph.char)) family.set(glyph.char, glyph.family);
}
...
for (const file of files) {
  ...
  console.log(
    '  glyphs: ' +
    [...histogram].sort((a, b) => b[1] - a[1]).map(([k, v]) => `${k} ${v}`).join('  '),
  );
}
