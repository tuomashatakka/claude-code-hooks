...
const SCRIPT: Beat[] = [
  ...
  tool('ask-user-question', 'session', 'AskUserQuestion',
       {
         questions: [{
           ...
           header:      'Store scope',
           multiSelect: false,
           options:     [{ label: 'Full: config into the app store (Recommended)', description: 'Move config into app state.' }],
         }],
       },
       { answers: { 'How far should the store rewrite go?': 'Full: config into the app store (Recommended)' }},
       0, {
         header: 'AskUserQuestion(Store scope)',
         note:   'The selected answer stays; the unused options, annotations and preview payload do not.',
       }),
  ...
  tool('bash-grep', 'shell', 'Bash',
    ...
    'Done in 42ms — see /tmp/rg.log\n',
       12, {
         ...
         note:   'Input and stdout share one card with distinct backgrounds; each ruler starts a fresh card below it.',
       }),
  ...
  tool('wcgw-bash', 'shell', 'mcp__wcgw__BashCommand',
    ...
    'This is the main shell. No command running in background.',
       2140, {
         ...
         note:   'wcgw appends a trailer to stdout; it is parsed into green/blue badges seated inside the card footer instead of trailing noise.',
       }),
  ...
  tool('apply-patch', 'files', 'apply_patch',
       ...
       100, {
         header: 'apply_patch(1 file)',
         note:   'Codex has already drawn the native per-file diff, so the hook keeps a successful apply_patch result quiet instead of duplicating it in another output card.',
       }),
  ...
  tool('view-image', 'files', 'view_image',
       { path: DEMO_PNG, detail: 'high' },
       { detail: 'high' },
       4, {
         header: 'view_image(docs/sigil.png)',
         note:   'Codex view_image uses the same fitted image card without leaking its raw data URL or generic metadata.',
       }),
  ...
  tool('pw-evaluate', 'web', 'mcp__playwright__browser_evaluate',
       { function: '() => ({ examples: window.__SESSION__.examples.length })' },
       {
         content: [{
           type: 'text',
           text: '{"examples": 42, "chapters": 6, "hash": "#read-image", "reduceMotion": false}',
         }],
         isError: false,
       },
       38, {
  ...
  tool('tool-search', 'web', 'ToolSearch',
       { query: 'select:WebFetch,ExitPlanMode', max_results: 3 },
       { matches: [ 'WebFetch', 'ExitPlanMode' ], query: 'select:WebFetch,ExitPlanMode', total_deferred_tools: 129 },
       4, {
  ...
  tool('update-plan', 'tasks', 'update_plan',
       {
         explanation: 'Keep the terminal refactor verifiable as it lands.',
         plan:        [
           { step: 'Map every tool payload', status: 'completed' },
           { step: 'Add semantic renderers', status: 'completed' },
           { step: 'Run terminal QA', status: 'in_progress' },
         ],
       },
       { ok: true },
       2, {
         header: 'update_plan(3 steps)',
         note:   'Plans and todo updates render as a compact state list with one completion badge.',
       }),
  ...
  tool('collaboration-spawn', 'tasks', 'collaborationspawn_agent',
       { task_name: 'lint_core_fix', agent_type: 'worker', message: 'Fix the core lint failures without touching unrelated files.' },
       { task_name: '/root/lint_core_fix' },
       4, {
         header: 'collaboration ▸ spawn agent(lint_core_fix)',
         note:   'Spawning a collaborator keeps its identity and role while dropping the duplicated private brief.',
       }),

  tool('collaboration-wait', 'tasks', 'collaborationwait_agent',
       { timeout_ms: 30_000 },
       { message: 'Wait timed out.', timed_out: true },
       30_000, {
         header: 'collaboration ▸ wait agent(30s)',
         note:   'A wait timeout becomes one quiet lifecycle line instead of a boxed JSON echo.',
       }),

  tool('collaboration-followup', 'tasks', 'collaborationfollowup_task',
       { target: '/root/lint_core_fix', message: 'Finish the remaining complexity warnings.' },
       {},
       2, {
         header: 'collaboration ▸ followup task(/root/lint_core_fix)',
         note:   'Follow-ups confirm the target without replaying their instruction body.',
       }),
  ...
  tool('task-stop', 'tasks', 'TaskStop',
       { task_id: 'agent-ab9e5c61' },
       JSON.stringify({
         message:   'Successfully stopped task: agent-ab9e5c61 (long command omitted by renderer)',
         task_id:   'agent-ab9e5c61',
         task_type: 'local_bash',
         command:   'bun test --watch --verbose',
       }),
       3, {
         header: 'TaskStop(agent-ab9e5c61)',
         note:   'Stopping a task keeps the id and task type while dropping the duplicated command body.',
       }),
...
if (duplicateId)
  throw new Error(`duplicate beat id "${duplicateId}" — hashes must be unique`)
...
for (const beat of SCRIPT) {
  ...
  if (code !== 0)
    throw new Error(beat.event + ' exited ' + code)
...
const chapters = CHAPTERS.map(({ id, label }) => {
  ...
  if (start === -1)
    throw new Error(`chapter "${id}" has no beats`)
