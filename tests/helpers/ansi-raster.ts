...
import { glyphTable, type Glyph } from '@tuomashatakka/image-to-ascii';
...
export function parseAnsi(text: string): AnsiCell[][] {
  ...
  return text.split('\n').map(line => {
    ...
    const emit = (chunk: string) => {
      for (const char of chunk) cells.push({ char, fg, bg });
    };
    ANSI_RE.lastIndex = 0;
    ...
    while ((m = ANSI_RE.exec(line))) {
      ...
      while (i < params.length) {
        ...
        if (code === 38 || code === 48) {
          ...
          if (kind === 5) { color = palette[params[i + 2]!] ?? null; i += 3; }
          else if (kind === 2) { color = { r: params[i + 2]!, g: params[i + 3]!, b: params[i + 4]! }; i += 5; }
          else i++;
          if (code === 38) fg = color; else bg = color;
        } else if (code === 39) { fg = null; i++; }
        else if (code === 49) { bg = null; i++; }
        else if (code === 0) { fg = null; bg = null; i++; }
        else i++;
      }
    }
    ...
    for (const mode of ['sextant', 'octant'] as const) {
      for (const palette of [false, true]) {
        for (const g of glyphTable(mode, palette).glyphs) {
          if (!charIndex.has(g.char)) charIndex.set(g.char, g);
        }
      }
    }
  }
...
function referenceSize(png: Buffer): { width: number; height: number } {
  const img = PNG.sync.read(png);
