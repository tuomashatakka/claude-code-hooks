...
function uiScreenshot (): Buffer {
  ...
  return build(w, h, (x, y) => {
    if (x === 240 || x === 241 || y === 48 || y === 49)
      return [ 90, 96, 110, 255 ] // 2px chrome rules
    if (x === 700)
      return [ 60, 64, 76, 255 ] // 1px hairline
    if (x < 240)
      return y % 64 < 28 && x > 24 && x < 210 ? [ 72, 78, 92, 255 ] : [ 34, 36, 44, 255 ]
    if (y < 48)
      return [ 28, 30, 38, 255 ]
...
function chart (): Buffer {
  ...
  return build(w, h, (x, y) => {
    ...
    if (Math.abs(y - curve) < 2.5)
      return [ 230, 90, 110, 255 ]
    if (Math.abs(y - (h - x * h / w)) < 2)
      return [ 70, 130, 230, 255 ] // 45-degree line
    if (Math.abs(y - (h - x * h / (w * 3))) < 2)
      return [ 40, 180, 140, 255 ] // shallow diagonal
  ...
  return build(w, h, (x, y) => {
    ...
    const ch = (c: number) => {
      let acc = 0,
        amp   = 0.6
      for (let o = 0; o < 4; o++) {
        acc += sample(o, c, u, v) * amp; amp *= 0.5
      }
