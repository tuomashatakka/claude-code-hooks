...
describe('renderFileResult', () => {
  test('titles the box with the path and seats the action inside its bottom-right corner', () => {
    const rendered = renderFileResult(target, { action: 'write' })!
    const lines    = rendered.split('\n')
    const out      = stripAnsi(rendered)
    const title    = stripAnsi(lines[1]!)
    // Last line is the trailing blank renderCard emits; the footer is the
    // backgrounded final row immediately before it.
    const bottom = stripAnsi(lines.at(-2)!)
    ...
    expect(bottom.trimEnd()).toEndWith('write')
