...
describe('highlightBash', () => {
  // The flag pass emits a truecolor escape whose parameters (38, 2, 224, 175)
  // each match \b\d+\b. Before replaceOutsideAnsi, the number pass that runs
  // after it colored those digits and the escape rendered as literal text.
  test('a flag plus a number does not shatter the flag escape', () => {
    const out = simpleHighlight('rg -n 42 src/index.ts', 'bash');
    expect(hasBrokenEscape(out)).toBe(false);
    expect(out).toContain('\x1b[38;2;224;175;104m-n');
